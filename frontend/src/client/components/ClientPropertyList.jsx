...
const ClientPropertyList = ({ clientID }) => {
  ...
  const noClient = !resolvedClientId;
    ...
    <section className="mx-auto max-w-6xl px-4 py-8">
      <div className="mb-6 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          {noClient && (
            <p className="text-xs text-gray-500 mt-1">No client selected — showing placeholder until client loads.</p>
          )}
        </div>
      ...
      {loading && properties.length === 0 ? (
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center text-sm text-gray-500 shadow-sm">
          {noClient ? 'No client selected.' : 'No properties found for this client.'}
        </div>
      ) : (
