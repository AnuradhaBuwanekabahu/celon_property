...
const DeleteLands = ({ clientID }) => {

    const navigate =useNavigate();

    const {  loading, lands,getlands ,deleteLands} = useContext(clientContext);
    ...
    const clientLand = lands.filter(
        (item) => Number(item.client_id) === Number(clientID)
    );


    // Search filter
    const filteredLands = clientLand.filter((item) => {
                                      ...
                                      <div className="absolute bottom-4 left-4 w-[260px] bg-white rounded-2xl p-4">
                                        ...
                                        <div className="flex items-center justify-between mt-3">
                                          ...
                                          <div className="flex items-center gap-1">
                                            <Trash2 className="text-red-800 size-4  "
                                            onClick={()=>deleteLands(item.id)}/>
                                          </div>
                                        </div>
