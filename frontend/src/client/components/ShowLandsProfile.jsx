...
const ShowLandsProfile = ({ landsID, onBack }) => {

    const { getlandsById, landsSelectedProperty } = useContext(clientContext);
    const { id: routeId } = useParams();
    const [showGallery, setShowGallery] = useState(false);
    const activeLandsId = landsID || routeId;
    const property = landsSelectedProperty?.data ?? landsSelectedProperty;

    useEffect(() => {
        if (activeLandsId) {
            getlandsById(activeLandsId);
        }
    }, [activeLandsId]);
