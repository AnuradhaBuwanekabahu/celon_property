...
import ShowStaytorent from './ShowStaytorent';
...
const ShowStaytorentProfile = ({ stayToBuyID, onBack }) => {

    const { staytorentpropterty,  getStayTorentById,} = useContext(clientContext);
    const { id: routeId } = useParams();
    const [showGallery, setShowGallery] = useState(false);
    const activeStayToBuyId = stayToBuyID || routeId;
    const property = staytorentpropterty?.data ?? staytorentpropterty;

    useEffect(() => {
        if (activeStayToBuyId) {
            getStayTorentById(activeStayToBuyId);
        }
    }, [activeStayToBuyId]);

    if (!property) return null;

    const otherImages = Array.isArray(property.gallery_images)
    ...
    return (
        <div className='w-full h-auto px-3 sm:px-6 lg:px-8 pb-8 max-w-5xl mx-auto'>
            ...
            <div className='flex flex-col sm:flex-row flex-wrap gap-x-6 gap-y-1.5 mt-3 text-xs sm:text-sm'>
                ...
                <p><span className='font-semibold text-[#14213D]'>Sq.Ft: </span>{property.area_sqft}</p>
                <p><span className='font-semibold text-[#14213D]'>Area of Land: </span>{property.land_area} perches</p>
            </div>

            {/* Overview */}
            {property.overview?.length > 0 && (
                <>
            ...
            {property.highlights?.length > 0 && (
                <>
                    ...
                    <div className='grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3'>
                        {property.highlights.map((feature, idx) => (
...
export default ShowStaytorentProfile