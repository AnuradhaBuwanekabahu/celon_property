...
const ShowStayBuyProfile = ({ stayToBuyID, onBack }) => {

    const { getStayToBuyById, staytoBuySelectedProperty } = useContext(clientContext);
    const { id: routeId } = useParams();
    const [showGallery, setShowGallery] = useState(false);
    const activeStayToBuyId = stayToBuyID || routeId;
    const property = staytoBuySelectedProperty?.data ?? staytoBuySelectedProperty;

    useEffect(() => {
        if (activeStayToBuyId) {
            getStayToBuyById(activeStayToBuyId);
        }
    }, [activeStayToBuyId]);
...
export default ShowStayBuyProfile