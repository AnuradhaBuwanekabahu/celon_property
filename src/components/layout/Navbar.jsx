      ...
      <div className="bg-black text-white">
        <div className="max-w-7xl mx-auto px-4 h-14 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            <button className="bg-secondary text-black font-semibold px-6 py-2.5 rounded-full text-sm hover:opacity-90 transition">
              Post Your Add
            </button>
    ...
    <div className="hidden md:flex justify-between w-full max-w-4xl text-sm">
      {menuLinks.map((link) => (
        <Link
          key={link.name}
          to={link.path}
          className="hover:text-secondary transition whitespace-nowrap"
        >
          {link.name}
        </Link>
      ))}
